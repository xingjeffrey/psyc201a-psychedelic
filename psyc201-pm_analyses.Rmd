...
ggsave(filename = 'f1rep.pdf', width = 9, height = 4, dpi = 300)
```

## Hypothesis 2:
Positive PM Perception = PM Use + Error
...
ggsave(filename = 'f2rep.pdf', width = 4, height = 4, dpi = 300)
```
...
ggsave(filename = 'f3rep.pdf', width = 6, height = 4, dpi = 300)
```
...
PM_cleaned %>%
  ...
  stat_smooth(method="glm", method.args = list(family=binomial), size = 2) +
  labs(
    title = "Older males are more likely to use more than older females",
    x = "Age",
...
ggsave(filename = 'f4nov.pdf', width = 6, height = 4, dpi = 300)
```